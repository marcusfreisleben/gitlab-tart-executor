...
const systemsetupListtimezones = `Africa/Abidjan
Africa/Accra
Africa/Addis_Ababa
Africa/Algiers
Africa/Asmara
...
func TestParseTimezoneAuto(t *testing.T) {
	_, err := timezone.Parse("auto")
...
func TestParseTimezoneNormal(t *testing.T) {
	for _, rawTZ := range strings.Split(strings.TrimSpace(systemsetupListtimezones), "\n") {
		t.Run(rawTZ, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, rawTZ, tz)
		})
	}
}

func TestParseTimezoneInvalid(t *testing.T) {
	_, err := timezone.Parse(";cat /etc/passwd")
	require.ErrorIs(t, err, timezone.ErrParseFailed)
}
